...
import { deleteFileFromLocal } from '../../../lib/localFileStorage';
...
async function updateComponent(req: NextApiRequest, res: NextApiResponse, id: string) {
  try {
    ...
    // Get the current component to check for media changes
    const { data: currentComponent, error: fetchError } = await db.supabase
      .from('components')
    ...
    // Check if media files have changed and delete old ones if needed
    if (currentComponent.preview_image && currentComponent.preview_image !== previewImage) {
      // Delete old image file if it's a local file
      if (currentComponent.preview_image.startsWith('/uploads/')) {
        await deleteFileFromLocal(currentComponent.preview_image);
      }
    }
    
    if (currentComponent.preview_video && currentComponent.preview_video !== previewVideo) {
      // Delete old video file if it's a local file
      if (currentComponent.preview_video.startsWith('/uploads/')) {
        await deleteFileFromLocal(currentComponent.preview_video);
      }
    }
...
async function deleteComponent(req: NextApiRequest, res: NextApiResponse, id: string) {
  try {
    // Get component details to find media files
    const { data: component, error: fetchError } = await db.supabase
      .from('components')
      .select('preview_image, preview_video')
      .eq('id', id)
      .single();
    ...
    if (component) {
      if (component.preview_image && component.preview_image.startsWith('/uploads/')) {
        await deleteFileFromLocal(component.preview_image);
      }
      
      if (component.preview_video && component.preview_video.startsWith('/uploads/')) {
        await deleteFileFromLocal(component.preview_video);
      }
    }
